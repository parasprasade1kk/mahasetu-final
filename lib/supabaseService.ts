...
export async function findCitizenByMobile(mobile: string) {
...
export async function registerCitizen({
  ...
  if (existingMobile) {
    const err: any = new Error(
      'This mobile number is already associated with an existing account. Please log in using your existing account.'
    );
    err.status = 409;
    throw err;
  }
  ...
  if (existingAadhaar) {
    const err: any = new Error(
      'This Aadhaar number is already associated with an existing account. Please log in using your existing account.'
    );
    err.status = 409;
    throw err;
  }
...
export async function authenticateCitizen({
  ...
  if (!citizen) {
    const err: any = new Error(
      'No account found for this mobile number. Please create a new account.'
    );
    err.status = 404;
    throw err;
  }
  ...
  if (aadhaar) {
    ...
    if (
      ...
      (!citizen.aadhaar_hash || citizen.aadhaar_masked?.endsWith('0000'))
    ) {
...
export async function submitApplication(appData: any) {
...
export async function updateApplicationStatus({
  ...
  status,
  remarks,
  changedBy = 'Government Administrator',
}: {
...
export async function getDocumentsByCitizen(userId: string) {
  const { data, error } = await supabase
    .from('documents')
    .select('*')
    .eq('user_id', userId)
...
export async function syncDigiLockerForCitizen(userId: string) {
...
export async function getConsentsByCitizen(userId: string) {
  const { data, error } = await supabase
    .from('consents')
    .select('*')
    .eq('user_id', userId)
...
export async function toggleConsent(consentId: string, userId: string) {
