...
import { Scheme, SchemeDocument, StructuredEligibility, ALL_SCHEMES } from './schemeDatabase';

export interface UserProfile {
  category: string; // 'SC' | 'ST' | 'OBC' | 'VJNT' | 'SBC' | 'EWS' | 'OPEN'
  annualIncome: number; // numeric in ₹
  age: number; // in years
  occupation: string; // 'student' | 'farmer' | 'woman' | 'senior' | 'destitute' | 'salaried' | 'unemployed'
  educationLevel: string; // 'School' | 'Diploma' | 'Undergraduate' | 'Postgraduate' | 'None'
  isStudent: boolean;
  hasDisability: boolean;
  isMaharashtraResident: boolean;
  gender: 'male' | 'female' | 'other' | 'any';
  district: string;
}

export type EligibilityStatus = 'eligible' | 'possible' | 'ineligible';
...
export interface EvaluatedScheme {
  ...
  passedReasonsEn: string[];
  passedReasonsMr: string[];
  failedReasonsEn: string[];
  failedReasonsMr: string[];
  missingDocuments: SchemeDocument[];
  availableDocuments: SchemeDocument[];
...
export function normalizeCategory(cat: string): string {
  ...
  if (['SC', 'SCHEDULED CASTE', 'SCHEDULED_CASTE'].includes(c)) return 'SC';
  if (['ST', 'SCHEDULED TRIBE', 'SCHEDULED_TRIBE'].includes(c)) return 'ST';
  if (['OBC', 'OTHER BACKWARD CLASS', 'OTHER BACKWARD CLASSES'].includes(c)) return 'OBC';
  if (['VJNT', 'VJ/NT', 'NT', 'VIMUKTA JATI'].some(k => c.includes(k))) return 'VJNT';
  if (['SBC', 'SPECIAL BACKWARD CLASS'].includes(c)) return 'SBC';
  if (['EWS', 'ECONOMICALLY WEAKER SECTION'].includes(c)) return 'EWS';
  if (['OPEN', 'GENERAL', 'OPEN / GENERAL'].some(k => c.includes(k))) return 'OPEN';
  if (['MINORITY', 'MUSLIM', 'BUDDHIST', 'JAIN', 'CHRISTIAN', 'PARSI', 'SIKH'].some(k => c.includes(k))) return 'Minority';
  return c || 'OPEN';
}

export function normalizeIncome(val: string | number): number {
  if (typeof val === 'number') return isNaN(val) ? 0 : val;
  if (!val) return 0;
  const str = String(val).trim();
  if (str === 'under-2.5L') return 150000;
  if (str === '2.5L-8L') return 500000;
  if (str === 'above-8L') return 900000;
...
export const SCHEME_RULES: Record<string, StructuredEligibility> = {
...
const SERVICE_RECOMMENDATIONS: Record<string, { id: string; name: string; nameMr: string; dept: string; deptMr: string }> = {
  'DOC-INC': {
    id: 'REV-001',
...
export function evaluateSchemeEligibility(profile: UserProfile, scheme: Scheme): EvaluatedScheme {
  // Merge scheme's embedded rules or lookup from master table
  const rules: StructuredEligibility = scheme.structuredEligibility || SCHEME_RULES[scheme.id] || {
    incomeOperator: 'none',
    allowedCategories: ['SC', 'ST', 'OBC', 'VJNT', 'SBC', 'EWS', 'OPEN'],
  };
  ...
  const passedReasonsEn: string[] = [];
  const passedReasonsMr: string[] = [];
  const failedReasonsEn: string[] = [];
  const failedReasonsMr: string[] = [];
  ...
  if (rules.occupations && rules.occupations.length > 0 && !rules.occupations.includes('any')) {
    ...
    const occPass = rules.occupations.some(o => {
      ...
      if (o === 'student' && isStudent) return true;
      if (o === 'senior' && (userRole === 'senior' || userAge >= 60)) return true;
      if (o === 'woman' && (userRole === 'woman' || profile.gender === 'female')) return true;
  ...
  if (hardFails > 0) {
    ...
    let score = 88;
    if (rules.disabilityRequired && profile.hasDisability) score += 12;
    if (rules.minAge && rules.minAge >= 60 && profile.age >= 60) score += 12;
    if (rules.studentRequired && isStudent) score += 10;
    if (rules.allowedCategories && !rules.allowedCategories.includes('OPEN') && rules.allowedCategories.includes(normUserCat)) score += 8;
    if (rules.gender && (rules.gender === profile.gender || (rules.gender === 'female' && profile.occupation === 'woman'))) score += 7;
    if (rules.occupations && rules.occupations.includes('farmer') && profile.occupation === 'farmer') score += 10;
    if (rules.isService) score = Math.min(score, 85); // Direct welfare schemes outrank certificate issuance
...
export function evaluateAllSchemes(profile: UserProfile): EvaluatedScheme[] {
  const evaluated = ALL_SCHEMES.map(scheme => evaluateSchemeEligibility(profile, scheme));
